...
from qdrant_client.models import Payload, ScoredPoint
from qdrant.customers import search_qdrant_customers, search_qdrant_customers_directly
...
async def search_qdrant_customer_by_email_address(email_address: str) -> Dict:
    customers = search_qdrant_customers_directly({"email": email_address})
    return get_customer_details(customers[0].payload)
...
async def search_customers(query: str) -> List[Dict]:
    ...
    for customer_raw in customers_raw:
        customers.append(get_customer_details(customer_raw.payload))

    return customers


def get_customer_details(customer_raw: Payload | None) -> Dict:
