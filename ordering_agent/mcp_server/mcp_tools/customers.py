...
from mcp_utils.customers import search_customer, search_qdrant_customer_by_email_address
...
class CustomerTools:
    ...
    async def search_customer_by_email(cls, ctx: Context, email_address: str):
        ...
        """
        customer = await search_qdrant_customer_by_email_address(email_address)
        print('This is a customer test')
        print(customer)
    ...
    async def customer_agreed_with_found_details(cls, ctx: Context):
        ...
        session_manager.found_addresses.extend(
            session_manager.customer.get('addresses', []))
