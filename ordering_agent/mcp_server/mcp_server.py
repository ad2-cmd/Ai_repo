...
mcp.tool(CustomerTools.customer_agreed_with_found_details,
         tags={'customer_identification'})
mcp.prompt(CustomerPrompts.search_customer_by_email_address,
           tags={'customer_identification'})
mcp.prompt(CustomerPrompts.customer_identification,
           tags={'customer_identification'})

# --- Products ---
mcp.tool(ProductTools.add_selected_products_to_cart,
         tags={'product_selection'})
mcp.prompt(ProductPrompts.search_for_products, tags={'product_selection'})
mcp.prompt(ProductPrompts.product_added_successfully,
           tags={'product_selection'})

# --- Shipping Method ---
mcp.tool(ShippingMethodTools.store_selected_shipping_method,
         tags={'shipping_method_selection'})
mcp.prompt(ShippingMethodPrompts.select_shipping_method_by_name,
           tags={'shipping_method_selection'})
mcp.prompt(ShippingMethodPrompts.list_shipping_methods,
           tags={'shipping_method_selection'})

# --- Shipping Address ---
mcp.tool(ShippingAddressTools.store_selected_shipping_address,
         tags={'shipping_address_selection'})
mcp.prompt(ShippingAddressPrompts.customer_address_selection,
           name="customer_shipping_address_selection", tags={'shipping_address_selection'})
mcp.prompt(ShippingAddressPrompts.request_close_location_to_gls_parcel_machine, tags={
           'shipping_address_selection'})
mcp.prompt(ShippingAddressPrompts.gls_parcel_machine_location_selection, tags={
           'shipping_address_selection'})

# --- Payment Address ---
mcp.prompt(PaymentAddressPrompts.customer_address_selection,
           name="customer_payment_address_selection", tags={'payment_address_selection'})
mcp.prompt(PaymentAddressPrompts.ask_for_new_payment_address,
           tags={'payment_address_selection'})
...
mcp.tool(PaymentMethodTools.store_selected_payment_method,
         tags={'payment_method_selection'})
mcp.prompt(PaymentMethodPrompts.list_payment_methods,
           tags={'payment_method_selection'})
...
mcp.prompt(
    ...
    tags={'order_finalization'}
)

# --- Session ---
# mcp.tool(SessionTools.set_order_state, tags={
#          'customer_identification', 'product_selection', 'shipping_method_selection', 'shipping_address_selection', 'payment_address_selection', 'payment_method_selection', 'order_confirmation', 'order_finalization'})
# mcp.tool(SessionTools.get_order_state, tags={
#          'customer_identification', 'product_selection', 'shipping_method_selection', 'shipping_address_selection', 'payment_address_selection', 'payment_method_selection', 'order_confirmation', 'order_finalization'})
...
@mcp.custom_route("/session_manager/session/{session_id}", methods=["POST"])
async def update_session(request):
    session_id = request.path_params["session_id"]
    if not session_id:
        return JSONResponse({"error": "No session_id provided"}, status_code=400)
    ...
    # Update order_state if provided
    if "order_state" in data:
        session_manager.order_state = data["order_state"]

    # You could also allow updating other keys if needed:
    # for key, value in data.items():
    #     session_manager.state[key] = value
